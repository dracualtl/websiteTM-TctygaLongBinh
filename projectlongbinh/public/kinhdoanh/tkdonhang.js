...
function themngay(){
   ...
   // document.getElementsByClassName("txtNSX")[0].value=date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate();
    ...
    document.getElementById("chonngay").valueAsDate=new Date(date.getFullYear(),date.getMonth(),date.getDate()-8);
    document.getElementById("chonngaykt").valueAsDate=new Date();
    ...
    document.getElementById("tungay").style.display="block";
    document.getElementById("denngay").style.display="block";
    ...
    $.get("http://localhost:3000/laythongtindonhang1",function(data){
        var dsma=[];
        var dsidkh=[];
        var dstt=[];
        var dstttt=[];
        ...
        for(var i=0;i<dsma.length;i++){
            ...
            //         <th><center>`+sohuy+`</center></th>
            //         <th><center>`+sodangban+`</center></th>
            //         <th>`+tongtien+`</th>
            //         </tr>`);
            if(tongtien==0){
                if(x==0 && sohuy==0 && sodangban==0){
                    $("#tablesoluong").append(`<tr>
                    <th>`+(i+1)+`</th>
                    <th>`+ngaylay.toLocaleDateString()+`</th>
                    <th></th>
                ...
                }else if(x==0 && sohuy==0 && sodangban!=0){
                    $("#tablesoluong").append(`<tr>
                    <th>`+(i+1)+`</th>
                    <th>`+ngaylay.toLocaleDateString()+`</th>
                    <th></th>
                ...
                var tongtien1=""+tongtien;
                    var mangtt=[];
                    var tongtien3=[];
                    var tongtien2="";
                    if(tongtien1.length>=4){
                        for(var k=tongtien1.length-1;k>=0;k--){
                            mangtt.push(tongtien1.substr(k,1));
                        }
                        for(var h=0;h<mangtt.length;h++){
                            var a=h+1;
                            tongtien3.push(mangtt[h]);
                            if(a%3==0 && a!=mangtt.length){
                                tongtien3.push(".");
                            }
                        }
                        for(var k=tongtien3.length-1;k>=0;k--){
                            tongtien2=tongtien2+tongtien3[k];
                            if(k==0){
                                tongtien2=tongtien2+" VND";
                            }
                        }
                    }
                if(x==0 && sohuy==0 && sodangban==0){
...
function batdauloc(){
    $.get("http://localhost:3000/laythongtindonhang",function(data){
        ...
        if(document.getElementById("btntheongay").disabled==true){
            if(Number(Math.ceil((new Date(document.getElementById("chonngaykt").value)-new Date(document.getElementById("chonngay").value))/(24*60*60*1000))+1)<1){
            ...
            var num = Number(Math.ceil((new Date(document.getElementById("chonngaykt").value)-new Date(document.getElementById("chonngay").value))/(24*60*60*1000))+1);
            for(var i=0;i<num;i++){
                var date= new Date(document.getElementById("chonngaykt").value);
                var ngaylay = new Date(date.getFullYear(),date.getMonth(),date.getDate()-i);
                var x=0;
                var sohuy=0;
                var sodangban=0;
...
function chontheongay(){
    $.get("http://localhost:3000/laythongtindonhang",function(data){
        ...
        $("#tablesoluong").html(`<tr>
            ...
            for(var i=0;i<num;i++){
                var date= new Date(document.getElementById("chonngay").value);
                var date1= new Date(document.getElementById("chonngaykt").value);
                var ngaylay = new Date(date1.getFullYear(),date1.getMonth(),date1.getDate()-i);
...
function chontheothang(){
    ...
    document.getElementById("btntheonam").disabled=false;
    document.getElementById("tungay").style.display="none";
    document.getElementById("denngay").style.display="none";
...
//               </tr>`);
//                 var date=new Date();
//                 document.getElementsByClassName("txtHSD")[i].value=date.getFullYear()+"-"+(date.getMonth()+1)+"-"+date.getDate();
//             }
